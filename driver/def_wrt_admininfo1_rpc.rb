...
*Description*
  Validate the Users Admin configuration
     - Edge
     - Boundary
...
#Launch the Users Admin ruby script
...
begin 
  ...
  while(row <= rows)
    ...
    g.admin_name.set((ws.Range("k#{row}")['Value']).to_s)
    g.admin_pswd.set((ws.Range("l#{row}")['Value']).to_s)
    g.admin_pswd2.set((ws.Range("m#{row}")['Value']).to_s)
